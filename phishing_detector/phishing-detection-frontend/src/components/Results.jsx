      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="card p-12 text-center">
          ...
          <p className="text-red-600 text-lg">
            Unable to perform the scan. Please check your input and try again.
          </p>
        </div>
      </div>
    );
  ...
  const handleDownload = () => {
    try {
      ...
      if (details.length > 0) {
        ...
        const detailData = details.map((detail, idx) => [
          idx + 1,
          typeof detail === 'string'
            ? detail
            : `${detail.source || 'Unknown'} - ${detail.safe ? 'Safe' : 'Suspicious'}`,
        ]);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
          ...
          <div className="flex items-center space-x-6">
            ...
            <div>
              <h1 className="text-4xl font-bold mb-2">
                {verdict ? `Scan Result: ${verdict}` : 'Scan Result'}
              </h1>
              <p className="text-xl opacity-90">
                {riskConfig.level} • {(score * 100).toFixed(1)}% Confidence
              </p>
            </div>
          </div>
          ...
          <ul className="list-disc ml-5 text-slate-700 space-y-2">
            {details.map((detail, idx) => (
              <li key={idx} className="flex items-center space-x-2">
                ...
                ></span>
                <span>
                  <strong>{detail.source || 'Unknown'}:</strong>{' '}
                  {detail.safe ? 'Safe ✅' : 'Suspicious ⚠️'}
                </span>
              </li>
            ))}
          </ul>
      ...
      <div className="flex flex-col sm:flex-row gap-4 pt-6">
        ...
        <button className="btn btn-secondary btn-lg flex-1" onClick={handleDownload}>
          <span className="flex items-center space-x-2">
            <span>📥</span>
